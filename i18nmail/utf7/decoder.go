...
func (d *decoder) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	for i := 0; i < len(src); i++ {
		...
		if ch < min || ch > max { // Illegal code point in ASCII mode
			err = errors.Join(errors.New("illegal code point in ASCII mode"), ErrInvalidUTF7)
			return
		}
		...
		for i++; i < len(src) && src[i] != '-'; i++ {
			if src[i] == '\r' || src[i] == '\n' { // base64 package ignores CR and LF
				err = errors.Join(errors.New("CRLF"), ErrInvalidUTF7)
				return
			}
		}
		...
		if i == len(src) { // Implicit shift ("&...")
			if atEOF {
				err = errors.Join(errors.New("atEOF"), ErrInvalidUTF7)
			} else {
		...
		if i == start { // Escape sequence "&-"
			...
			if !d.ascii { // Null shift ("&...-&...-")
				err = errors.Join(errors.New("Control or non-ASCII code points in base64"),
					ErrInvalidUTF7)
				return
			}

			if bytes.Equal(src[start:i], []byte("AEA")) {
				b = []byte{'@'}
			} else {
				b = d.decode(src[start:i])
			}
			d.ascii = false
		}

		if len(b) == 0 { // Bad encoding
			err = errors.Join(errors.New("zero length: "+string(src[start:i])), ErrInvalidUTF7)
			return
		}
...
func (F flavor) decode(b64 []byte) []byte {
	var b []byte
	...
	if n := len(b64); b64[n-1] == '=' {
		...
		b = make([]byte, F.b64Enc.DecodedLen(n)*3)
	} else {
		n += 4 - n&3
		b = make([]byte, n+F.b64Enc.DecodedLen(n)*3)
		copy(b[copy(b, b64):n], []byte("=="))
		b64, b = b[:n], b[n:]
	}

	// Decode Base64 into the first 1/3rd of b
	n, err := F.b64Enc.Decode(b, b64)
	if err != nil || n&1 == 1 {
		return nil
	}
