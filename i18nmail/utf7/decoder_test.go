...
var decode = []struct {
	...
	// Real-world
	{"H&ANY-B Nemzetk&APY-zi (nemzetkozi&AEA-hob)", "HÖB Nemzetközi (nemzetkozi@hob)", true},
}

func TestModifiedDecoder(t *testing.T) {
	for _, test := range decode {
		out, err := utf7.Modified.NewDecoder().String(test.in)
		if out != test.out {
...
var m2g = strings.NewReplacer(
	"&/+8-", "+,+8-",
	"&", "+", ",", "/")

func TestGenuineDecoder(t *testing.T) {
	for _, test := range decode {
		test.in, test.out = m2g.Replace(test.in), m2g.Replace(test.out)
		out, err := utf7.Genuine.NewDecoder().String(test.in)
		if out != test.out {
			t.Errorf("UTF7Decode(%+q) expected %+q; got %+q", test.in, test.out, out)
		}
		if test.ok {
