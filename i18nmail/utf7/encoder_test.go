...
var encode = []struct {
	...
	// Real-world
	{"HÖB Nemzetközi (nemzetkozi@hob)", "H&ANY-B Nemzetk&APY-zi (nemzetkozi@hob)", true},
}

func TestModifiedEncoder(t *testing.T) {
	for _, test := range encode {
		out, err := utf7.Modified.NewEncoder().String(test.in)
		if err != nil {
			t.Error(err)
		}
		if out != test.out {
...
func TestGenuineEncoder(t *testing.T) {
	for _, test := range encode {
		test.in, test.out = m2g.Replace(test.in), m2g.Replace(test.out)
		out, err := utf7.Genuine.NewEncoder().String(test.in)
		if err != nil {
			t.Error(err)
		}
		if out != test.out {
